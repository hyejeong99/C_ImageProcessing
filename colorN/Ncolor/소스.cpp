#include <stdio.h>
#include <Windows.h>
#define WIDTHBYTES(bits) (((bits)+31)/32*4) // 가로길이는 4바이트 배수
...
void main() {
	...
	FILE *infile, *outfile;
	//영상데이터 읽기
	err = fopen_s(&infile, "pshop256.bmp", "rb");
	if (infile == NULL) { printf("There is no file!!!\n"); exit(1); }

	BITMAPFILEHEADER hf;
	fread(&hf, sizeof(BITMAPFILEHEADER), 1, infile); // 파일헤드
	if (hf.bfType != 0x4D42) exit(1); // String “BM” BITMAPINFOHEADER hInfo;
	...
	else {// 입력영상이 트루 칼라가 아닌 경우
		for (i = 0; i < hInfo.biHeight; i++)
		{
			for (j = 0; j < hInfo.biWidth; j++)
			{
				index = lpImg[i*rwsize + j]; // 팔레트의 인덱스
				R = pRGB[index].rgbRed; // R
	...
	// 영상 출력
	err = fopen_s(&outfile, "OutImg24.bmp", "wb");
	if (outfile == NULL) { printf("Error!!, Can not write to disk\n"); exit(1); }
